...
$(document).ready(function() {
	/* =========== NUEVAS VALIDACIONES ============= */
	jQuery.validator.addMethod("numberSpace", function(value, element) {
		return this.optional(element) || /^[0-9,\s]+$/i.test(value);
	}, "Números y espacios (73713 1734)");
	jQuery.validator.addMethod("noSpace", function(value, element) { 
		  return value.indexOf(" ") < 0 && value != ""; 
		}, "No se permiten espacio en este campo");
	/* =========== NUEVAS VALIDACIONES ============= */
	clearFormButton();
	changeUser('Mavitours.logged', 'img/avatars/male.png');
	...
	$('#logout a').click(function(e) {
		...
		$.SmartMessageBox({
			...
			content: $.logoutMSG || "Usted puede mejorar su seguridad aún más después de cerrar la sesión al cerrar este navegador abierto",
			buttons: '[Si][No]'

		}, function(ButtonPressed) {
			if (ButtonPressed == "Si") {
				$.getJSON("js/services/logout.json", function(response, textStatus, xhr) {
 ...
 * Fecha 2014-05-27
 * Argumentos id del formulario
 * Descripción Función para limpiar los campos del formulario
 * 
 **/
 ...
 * Autor William Alarcon
 * Fecha 2014-07-14
 * Descripción Obtener un objeto JSON las ciudades de colombia
 * 
 *
 **/
function getCities() {
	$.getJSON("js/services/cities.json", function(response) {
		$.each(response.datos, function(key, dato) {
			$('#cities').append('<option value="' + dato['nombre'] + '">' + dato['nombre'] + '</option>');
		});
	});
}
...
function postResourcesSelect(service, selectId, data) {
	...
	$('#' + selectId).html('');
	$.post("js/services/" + service + ".json", data, function(response) {
		$.each(response.datos, function(key, dato) {
			$('#' + selectId).append('<option value="' + dato['nombre'] + '">' + dato['nombre'] + '</option>');
		});
	});
}
 ...
 * Autor William Alarcon
 * Fecha 2014-08-13
 * Descripción agregar dos años mas a los campos de fecha 
 * 
 **/
function setYear(inpuId, moreYear) {
	var d = new Date();
	if (typeof moreYear === "undefined") {
		moreYear = 0;
	}
	$('#' + inpuId).attr('min', 1900);
	$('#' + inpuId).attr('max', (d.getFullYear()) + moreYear);
}
/*
 * Autor William Alarcon
 * Fecha 2014-08-13
 * Descripción definir el mes actual a un campo especifico
 * 
 **/
function setMonth(inpuId) {
	var d = new Date();
	$('#' + inpuId).attr('max', d.getMonth());
	$('input[name=' + inpuId + ']').attr('max', (d.getMonth() + 1));
//	var y = 2014;
//	var m = 8;
...
function deletedChildren() {
	...
	$('#deleteAllChildrens').click(function() {
		var tableId = '#' + $(this).parents('table').attr('id');
		alert(tableId);
		if ($(this).is(':checked')) {
			$.each($(tableId + ' tbody tr'), function(key, dato) {
	...
	$(document).on('click', '#deleteChildrens', function(event) {
		event.preventDefault();
		var tableId = '#' + $(this).parents('.content-table').find('table').attr('id');
		$.each($(tableId + ' tbody tr'), function(key, dato) {
		...
		$('.setCustom-scroll table' + tableId).removeAttr('style');
		if ($('.setCustom-scroll table' + tableId).height() > 221) {
			$('.setCustom-scroll table' + tableId).addClass('custom-scroll table-responsive').css({'height': '222px', 'overflow-y': 'scroll'});
		} else {
			if ($('.setCustom-scroll table' + tableId).hasClass('custom-scroll')) {
				$('.setCustom-scroll table' + tableId).removeClass('custom-scroll table-responsive ').removeAttr('style');
				$('.setCustom-scroll table' + tableId).css('border-width', 0);
 ...
 * Autor William Alarcon
 * Fecha 2014-27
 * Descripción Orderna los selects alfabéticamente
 * 
 **/
function orderingSelect(id_componente)
{
	var selectToSort = $('#' + id_componente);
	var optionCurrent = selectToSort.val();
...
function customFunction() {
	jQuery.validator.addMethod("money", function(value, element) {
		return this.optional(element) || /^\d{0,3}(\.\d{0,3})(\.\d{0,3})?$/.test(value);
	}, "Debe estar en  es-CO formato 999.999.999");
 ...
 * Autor William Alarcon
 * Fecha 2014-09-11
 * Descripción Validar campo de fecha
 *			   Para que esta función sirva se debe colocar al contendor del label y el input la clase container_date y el input 
 *			   debe tener la clase dateValidate
 * 
 **/
...
function getValidateDate() {
	$('.dateValidate').on({
		keyup: function() {
			$(this).parents('.container_date').find('.invalid').remove();
			var value = $(this).val();
...
function validateDate(input, value) {
	...
	input.parents('.container_date').find('.invalid').remove();
	var partDate = value.split('/');
	if (partDate[0] !== '') {
		if (partDate[0].length < 2) {
			if (input.parents('.container_date').find('.invalid').length === 0) {
				input.parents('.container_date').append('<em class="invalid" style="color:#D56161;font-style: normal;clear: both;display: block;">El dia debe ser ej: 01</em>');
			}
			return false;
		}
		if (partDate[0] < 01 || partDate[0] > 31) {

			if (input.parents('.container_date').find('.invalid').length === 0) {
				$(this).parents('.container_date').append('<em class="invalid" style="color:#D56161;font-style: normal;clear: both;display: block;">El dia es incorrecto</em>');
			}
			return false;
		}
	...
	if (partDate[1] !== '') {
		if (partDate[1].length < 2) {
			if (input.parents('.container_date').find('.invalid').length === 0) {
				input.parents('.container_date').append('<em class="invalid" style="color:#D56161;font-style: normal;clear: both;display: block;">El dia debe ser ej: 01</em>');
			}
			return false;
		}
		if (partDate[1] < 1 || partDate[1] > 12) {
			if (input.parents('.container_date').find('.invalid').length === 0) {
				input.parents('.container_date').append('<em class="invalid" style="color:#D56161;font-style: normal;clear: both;display: block;">El mes es incorrecto</em>');
			}
			return false;
		}
	...
	if (partDate[2] !== '') {
		if (partDate[2] < 1990 || partDate[2] > currentYear) {
			if (input.parents('.container_date').find('.invalid').length === 0) {
				input.parents('.container_date').append('<em class="invalid" style="color:#D56161;font-style: normal;clear: both;display: block;">El año es incorrecto</em>');
			}
			return false;
		} else {
			var y = partDate[2];
			...
			if (partDate[0] > d) {
				if (input.parents('.container_date').find('.invalid').length === 0) {
					input.parents('.container_date').append('<em class="invalid" style="color:#D56161;font-style: normal;clear: both;display: block;">El mes seleccionado tiene ' + d + ' días</em>');
				}
				return false;
			}
		}
